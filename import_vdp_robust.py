...
def safe_json_parse(params_str):
    """Safely parse JSON params string"""
    try:
        ...
        print(f"Problematic string: {params_str[:100]}...")
        return {}
...
def import_vdp_data():
    """Import the complete VDP data with all parameters using proper CSV parsing"""
    ...
    for row in connect_samples:
        print(f"  {row[0]} | Agent {row[1]} | {row[2]} | {row[3]} | {row[4]} | {row[5]}, {row[6]} {row[7]} | {row[8]}")
