 ...
 * Rules:
 *   - Cap: 300 pending non-Plus leads per agent
 *   - Only assigns pending non-Plus leads matching the agent's markets/states
 *   - Never touches Plus Leads or TaalkResolve=true leads
 */
...
app.post("/api/ccpro/assignment-trigger", async (req, res) => {
  ...
  try {
    ...
    // Step 1: Acquire per-agent advisory lock inside transaction — prevents race conditions
    // where concurrent requests all see 0 leads and each assign 50, causing 200+ leads
    const lockResult = await client.query(`SELECT pg_try_advisory_xact_lock(hashtext($1))`, [email]);
    const gotLock = lockResult.rows[0]?.pg_try_advisory_xact_lock;
    if (!gotLock) {
      await client.query('ROLLBACK');
      console.log(`[assignment-trigger] ${email} — already processing, skipping duplicate request`);
      return res.json({ ok: true, assigned: 0, current: 0, message: 'Concurrent request skipped (lock held)' });
    }
