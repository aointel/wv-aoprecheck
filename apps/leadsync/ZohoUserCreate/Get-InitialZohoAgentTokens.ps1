# Configuration
$config = @{
    client_id     = "1000.J997V5CGG0NS742VQA401ZFKTI5MJF"
    client_secret = "b60187effb9b211d02853c974b5f82ae3f51e53f54"
    code          = "1000.faa7fb4aaef464d944eec147deeab3f3.52835989add1ffb1732304f046d92923"  # Need new code
    redirect_uri  = "http://localhost:8080/callback"
    accounts_url  = "https://accounts.zoho.com"
}

# Use the agents scope
$scope = "ZohoVoice.agents.READ"

Write-Host "`nGetting tokens for Agents scope..."

$tokenUrl = "$($config.accounts_url)/oauth/v2/token"
$body = @{
    code          = $config.code
    client_id     = $config.client_id
    client_secret = $config.client_secret
    grant_type    = "authorization_code"
    redirect_uri  = $config.redirect_uri
    scope         = $scope
}

try {
    $response = Invoke-RestMethod -Uri $tokenUrl -Method Post -Body $body

    $tokens = @{
        access_token  = $response.access_token
        refresh_token = $response.refresh_token
        expires_at    = (Get-Date).AddSeconds($response.expires_in).ToString('o')
        scope         = $scope
    }

    # Save tokens to agents file
    $fileName = "zoho_tokens_agents.json"
    $tokens | ConvertTo-Json | Set-Content -Path $fileName
    
    Write-Host "Tokens saved to $fileName"
}
catch {
    Write-Error "Failed to get tokens for Agents: $_"
} 