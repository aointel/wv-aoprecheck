...
export const callTrackingClient = {
  ...
  async getCallHistory(
    ...
    const response = await fetch(`/api/call-history?userEmail=${encodeURIComponent(agentEmail || '')}&${params}`);
    if (!response.ok) {
  ...
  async getproducerMetrics(agentEmail: string, dateFrom: Date, dateTo: Date) {
    const params = new URLSearchParams();
