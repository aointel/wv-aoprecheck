...
export function TaalkCallInterface({ session, onCallComplete }: TaalkCallInterfaceProps) {
  ...
  const initiateCall = async () => {
    try {
      ...
      const response = await fetch(`/api/verification/session/${session.sessionId}/start-live-call`, {
        ...
        body: JSON.stringify({
          producerPhone: session.producerPhone || "+15032018470" // Use producer phone from Supabase profile
        })
      });
  ...
  const getStatusInfo = () => {
    switch (callStatus) {
      ...
      case 'connected':
        return { color: 'default', text: 'producer Connected', icon: CheckCircle };
      case 'completed':
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-2 gap-4 p-4 border rounded-lg mb-4">
            ...
            <div>
              ...
              <div className="space-y-1 text-sm text-gray-600">
                ...
                <p>2. producer receives call with client info</p>
                <p>3. Producer Calls client for verification</p>
                <p>4. producer takes screenshot during call</p>
                <p>5. producer confirms completion in system</p>
              </div>
            </div>
          </div>
          ...
          {callStatus === 'calling' && (
            <div className="text-center py-8 bg-blue-50 rounded-lg">
              ...
              <p className="text-sm text-blue-600">producer will receive call for {clientName} verification</p>
          ...
          {callStatus === 'connected' && (
            <div className="text-center py-8 bg-green-50 rounded-lg">
              ...
              <p className="text-lg font-medium text-green-800">producer Connected!</p>
