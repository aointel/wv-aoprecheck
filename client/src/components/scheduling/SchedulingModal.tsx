...
export default function SchedulingModal({ isOpen, onClose }: SchedulingModalProps) {
  ...
  // 🕒 AUTO-DETECT producer'S LOCAL TIMEZONE
  useEffect(() => {
    if (isOpen) {
      const localTimezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
      const timezoneAbbr = getTimezoneAbbreviation(localTimezone);
      setSelectedTimezone(timezoneAbbr);
      generateAvailableTimes(timezoneAbbr);
    }
  }, [isOpen]);
  ...
  const getTimezoneAbbreviation = (timezone: string): string => {
    ...
    return timezoneMap[timezone] || 'EST'; // Default to EST if not found
  };
  ...
  const generateAvailableTimes = (timezone: string) => {
    const times = [];
    for (let hour = 9; hour <= 17; hour++) {
      // 9 AM to 5 PM
      times.push(`${hour}:00 ${hour < 12 ? 'AM' : hour === 12 ? 'PM' : 'PM'}`);
      if (hour < 17) { // Don't add 5:30 PM
        times.push(`${hour}:30 ${hour < 12 ? 'AM' : hour === 12 ? 'PM' : 'PM'}`);
      }
    }
  ...
  const handleTimezoneChange = (timezone: string) => {
    ...
    generateAvailableTimes(timezone);
    // Clear selected time when timezone changes
    setNewAppointmentData(prev => ({ ...prev, time: '' }));
  };
  ...
  const createAppointmentMutation = useMutation({
    mutationFn: async (appointmentData: any) => {
      ...
      const enrichedAppointmentData = {
        ...
        timezoneOffset: getTimezoneOptions().find(tz => tz.value === selectedTimezone)?.offset || -5
      };
      return await apiRequest('POST', '/api/appointments', enrichedAppointmentData);
    },
    onSuccess: () => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={newAppointmentData.type} onValueChange={(value) => setNewAppointmentData(prev => ({ ...prev, type: value as any }))}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="recruit">AO Recruit - producer Interview</SelectItem>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Times shown in {selectedTimezone} timezone
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={newAppointmentData.time} onValueChange={(value) => setNewAppointmentData(prev => ({ ...prev, time: value }))}>
                      <SelectTrigger>
                        <SelectValue placeholder="Select your preferred appointment time" />
                      </SelectTrigger>
                      <SelectContent>
