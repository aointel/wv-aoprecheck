...
export function WebRTCCall({ sessionId, clientName, onCallComplete }: WebRTCCallProps) {
  ...
  const startCall = async () => {
    try {
      ...
      const wsUrl = `${protocol}//aoirail-production.up.railway.app/ws/phone-bridge`;
      const socket = new WebSocket(wsUrl);
      
      socket.onopen = () => {
        // Register this producer session with Twilio phone number
      ...
      socket.onmessage = (event) => {
        ...
        switch (message.type) {
          ...
          case 'incoming_call':
            ...
            // Auto-join producer to conference room
            const conferenceName = `AO-Verification-${sessionId}`;
            const conferenceWindow = window.open(`/api/conference/join/${conferenceName}?identity=producer`, '_blank');
