...
interface CallbackStats {
  ...
  producerSpecificCallbacks: Record<string, number>;
}
...
export function InboundCallMonitor() {
  ...
  return (
    <div className="space-y-4">
          ...
          {activeCallsData?.activeCalls && activeCallsData.activeCalls.length > 0 ? (
              ...
              {activeCallsData.activeCalls.map((call) => (
                <div key={call.id} className="border rounded-lg p-4 space-y-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
                    <div>
                      <span className="font-medium">producer:</span> {call.routedToproducer || 'Unassigned'}
                    </div>
                    <div>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                When leads call back, they'll be automatically routed to their original producer
              </p>
            </div>
          )}
        </CardContent>
      ...
      {/* producer-Specific Callback Distribution */}
      {statsData?.stats?.producerSpecificCallbacks && Object.keys(statsData.stats.producerSpecificCallbacks).length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Users className="h-5 w-5" />
              Callback Distribution by producer
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              {Object.entries(statsData.stats.producerSpecificCallbacks).map(([producer, count]) => (
                <div key={producer} className="flex items-center justify-between p-2 rounded border">
