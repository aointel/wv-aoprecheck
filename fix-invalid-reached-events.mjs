/**
 * Fix invalid reached events in agent_dial_metrics
 * This script batches through all reached events and removes/fixes invalid ones
 * 
 * Requirements for valid reached events:
 * - Must have call_sid (to verify against twilio_call_logs)
 * - Must have duration >= 30 seconds (from twilio_call_logs or agent_dial_metrics)
...
async function fixInvalidReachedEvents() {
  console.log('🔍 Starting fix for invalid reached events...\n');
  
  // Use a wide date range - include previous day at 00:00 UTC to catch early morning events
  const now = new Date();
  ...
  // Step 1: Fetch all reached events
  console.log('📊 Fetching all reached events...');
  
  const { data: reachedEvents, error: fetchError } = await supabase
  ...
  console.log(`✅ Found ${reachedEvents.length} reached events\n`);
  ...
  for (const event of reachedEvents) {
    ...
    const hasDuration = event.call_duration !== null && event.call_duration >= 30;
    ...
    // Validate: must have call_sid, NON-NULL duration, duration >= 30s, and status = 'answered'/'completed'
    const isValidStatus = effectiveStatus === 'answered' || effectiveStatus === 'completed';
    const hasValidDuration = effectiveDuration !== null && effectiveDuration !== undefined && effectiveDuration >= 30;
  ...
  if (toDelete.length > 0) {
    console.log(`🗑️  Deleting ${toDelete.length} invalid reached events...`);
  ...
  console.log(`   ✅ Valid reached events remaining: ${valid.length}`);
