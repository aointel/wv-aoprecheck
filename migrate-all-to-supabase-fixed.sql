-- COMPLETE SUPABASE MIGRATION SCRIPT - FIXED FOR UUID COMPATIBILITY
-- This migrates all ConnectNow tables from local PostgreSQL to Supabase
-- Fixed to handle Supabase's UUID primary key requirements

-- =====================================================
-- 1. USERS AND AUTHENTICATION
-- =====================================================

-- Users table (using UUID for Supabase compatibility)
...
-- Admin system (simplified without foreign keys)
...
-- Call center events (no foreign key to avoid conflicts)
CREATE TABLE IF NOT EXISTS call_center_events (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  event_type TEXT NOT NULL,
  call_id TEXT, -- Store as text instead of foreign key
...
-- Appointment reminders (no foreign key to avoid conflicts)
CREATE TABLE IF NOT EXISTS appointment_reminders (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  appointment_id TEXT NOT NULL, -- Store as text instead of foreign key
...
CREATE POLICY "Enable all access" ON hot_lead_notifications FOR ALL USING (true);
CREATE POLICY "Enable all access" ON hot_lead_callbacks FOR ALL USING (true);
...
CREATE POLICY "Enable all access" ON verification_sessions FOR ALL USING (true);
...
-- Success message
SELECT 'Supabase migration completed successfully! All ConnectNow tables created with UUID compatibility.' as migration_status;