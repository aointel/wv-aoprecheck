...
BEGIN
  ...
  IF NEW.company_email IS NOT NULL THEN
    ...
    IF NOT EXISTS (
      ...
      RAISE LOG 'Auto-creating user_credits for new customer: %', NEW.company_email;
      ...
      RAISE LOG 'Successfully auto-created user_credits for: %', NEW.company_email;
    ELSE
      RAISE LOG 'user_credits record already exists for: %, skipping creation', NEW.company_email;
    END IF;
  ELSE
    RAISE WARNING 'Cannot create user_credits: company_email is NULL for customer ID: %', NEW.id;
  END IF;
...
-- Step 5: Test query to verify function exists
SELECT 
  routine_name,
  routine_type
FROM information_schema.routines
WHERE routine_name = 'auto_create_user_credits_on_customer_insert';
