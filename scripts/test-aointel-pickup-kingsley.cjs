/**
 * Test AOIntel PICK_UP Event for Kingsley
 * 
 * This script simulates an AOIntel PICK_UP event for kingsleyibeh@aoglobelife.com
 * to test the AOIntel lead creation and dialer pausing flow.
 * 
 * Usage: node scripts/test-aointel-pickup-kingsley.cjs
 */

const { createClient } = require('@supabase/supabase-js');
const path = require('path');
const fs = require('fs');

// Load Supabase credentials
let SUPABASE_URL, SUPABASE_SERVICE_KEY;

// Try to load from hardcoded-config.ts
const hardcodedConfigPath = path.join(__dirname, '..', 'server', 'hardcoded-config.ts');
if (fs.existsSync(hardcodedConfigPath)) {
  const configContent = fs.readFileSync(hardcodedConfigPath, 'utf-8');
  
  const urlMatch = configContent.match(/SUPABASE_URL:\s*['"]([^'"]+)['"]/);
  if (urlMatch) SUPABASE_URL = urlMatch[1];
  
  const keyMatch = configContent.match(/SUPABASE_SERVICE_KEY:\s*['"]([^'"]+)['"]/);
  if (keyMatch) SUPABASE_SERVICE_KEY = keyMatch[1];
}

if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
  console.error('❌ Could not load Supabase credentials from hardcoded-config.ts');
  process.exit(1);
}

const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_KEY);

// Test data for Kingsley
const TEST_AGENT_EMAIL = 'kingsleyibeh@aoglobelife.com';

async function getAssociateId(email) {
  console.log(`🔍 Looking up associate_id for ${email}...`);
  
  // Try user_credits first
  const { data: user } = await supabase
    .from('user_credits')
    .select('associate_id')
    .eq('email', email.toLowerCase())
    .maybeSingle();
  
  if (user?.associate_id) {
    console.log(`✅ Found associate_id in user_credits: ${user.associate_id}`);
    return String(user.associate_id);
  }
  
  // Try producerlist
  const { data: producer } = await supabase
    .from('producerlist')
    .select('associate_id')
    .ilike('company_email', email)
    .maybeSingle();
  
  if (producer?.associate_id) {
    console.log(`✅ Found associate_id in producerlist: ${producer.associate_id}`);
    return String(producer.associate_id);
  }
  
  // Try customers table
  const { data: customer } = await supabase
    .from('customers')
    .select('associate_id')
    .ilike('company_email', email)
    .maybeSingle();
  
  if (customer?.associate_id) {
    console.log(`✅ Found associate_id in customers: ${customer.associate_id}`);
    return String(customer.associate_id);
  }
  
  console.warn('⚠️ Could not find associate_id - using fallback 999');
  return '999'; // Fallback
}

async function createTestPickup() {
  console.log('🧪 Creating test AOIntel PICK_UP event for kingsleyibeh@aoglobelife.com\n');
  console.log('='.repeat(60));
  
  // Get associate ID
  const associateId = await getAssociateId(TEST_AGENT_EMAIL);
  console.log(`\n📋 Using associate_id: ${associateId}\n`);
  
  // Generate unique test lead ID (must be numeric for vdp_calls table)
  const testLeadId = Math.floor(900000000 + Math.random() * 99999999); // Random 9-digit test ID
  const testPhone = `+1555${Math.floor(1000000 + Math.random() * 9000000)}`; // Random test phone
  
  // Test lead data - using "Veteran" market to trigger AOIntel
  const vdpCallData = {
    agent: associateId,
    event: 'PICK_UP',
    leadid: String(testLeadId), // Ensure it's a string
    company_email: TEST_AGENT_EMAIL,
    phone: testPhone,
    firstName: 'Test',
    lastName: 'AOIntel Lead',
    market: 'Veteran', // AOIntel market - triggers the poller (contains "veteran")
    duration: null,
    mga: null,
    rga: null,
    time: new Date().toISOString()
  };
  
  console.log('📝 VDP Call Data:');
  console.log(JSON.stringify(vdpCallData, null, 2));
  console.log('');
  
  // Insert into vdp_calls
  console.log('⏳ Inserting into vdp_calls table...');
  const { data: insertedCall, error: insertError } = await supabase
    .from('vdp_calls')
    .insert(vdpCallData)
    .select()
    .single();
  
  if (insertError) {
    console.error('❌ Failed to insert vdp_call:', insertError);
    console.error('   Details:', JSON.stringify(insertError, null, 2));
    return;
  }
  
  console.log('✅ VDP call inserted successfully!');
  console.log(`   ID: ${insertedCall.id}`);
  console.log(`   Lead ID: ${testLeadId}`);
  console.log(`   Phone: ${testPhone}`);
  console.log(`   Market: Veteran (AOIntel)`);
  console.log(`   Agent: ${TEST_AGENT_EMAIL}`);
  console.log(`   Associate ID: ${associateId}`);
  
  console.log('\n⏳ Waiting for AOIntel VDP Poller to process (10 seconds)...');
  console.log('   (Poller checks every 2 seconds, so this should be enough)');
  
  // Wait for poller to pick it up (poller checks last 30 seconds, so 10 seconds should be enough)
  await new Promise(resolve => setTimeout(resolve, 10000));
  
  // Check if masterlead was created
  console.log('\n🔍 Checking masterlead table for created lead...');
  const { data: createdLead, error: leadError } = await supabase
    .from('masterlead')
    .select('*')
    .eq('taalk_lead_id', String(testLeadId))
    .maybeSingle();
  
  if (leadError) {
    console.error('❌ Error checking masterlead:', leadError);
  } else if (createdLead) {
    console.log('✅ Lead created in masterlead!');
    console.log(`   ID: ${createdLead.id}`);
    console.log(`   taalk_lead_id: ${createdLead.taalk_lead_id}`);
    console.log(`   cn_email: ${createdLead.cn_email}`);
    console.log(`   cnresolution: ${createdLead.cnresolution}`);
    console.log(`   aointel: ${createdLead.aointel}`);
    console.log(`   taalk_lead_source: ${createdLead.taalk_lead_source || 'N/A'}`);
    console.log(`   first_name: ${createdLead.first_name}`);
    console.log(`   last_name: ${createdLead.last_name}`);
    console.log(`   phone: ${createdLead.phone}`);
  } else {
    console.log('⚠️ Lead not found in masterlead yet - poller may not have run yet');
    console.log('   Check server logs for AOIntel poller activity');
    console.log('   The poller should log: "📞 AOIntel PICK_UP: Found X AOIntel call(s)"');
  }
  
  // Check agent status
  console.log('\n🔍 Checking agent_live_call_status...');
  const { data: agentStatus, error: statusError } = await supabase
    .from('agent_live_call_status')
    .select('*')
    .eq('agent_email', TEST_AGENT_EMAIL.toLowerCase())
    .maybeSingle();
  
  if (statusError) {
    console.error('❌ Error checking agent status:', statusError);
  } else if (agentStatus) {
    console.log(`✅ Agent status: ${agentStatus.status}`);
    console.log(`   (Should be 'in_call' to pause outbound dialer)`);
    console.log(`   Last heartbeat: ${agentStatus.last_heartbeat_at}`);
  } else {
    console.log('⚠️ No agent status found - agent may not be online');
  }
  
  // Check live_call_boardt
  console.log('\n🔍 Checking live_call_boardt...');
  const { data: boardStatus, error: boardError } = await supabase
    .from('live_call_boardt')
    .select('status, current_call')
    .eq('agent_email', TEST_AGENT_EMAIL.toLowerCase())
    .maybeSingle();
  
  if (boardError) {
    console.error('❌ Error checking live_call_boardt:', boardError);
  } else if (boardStatus) {
    console.log(`✅ Live call board status: ${boardStatus.status}`);
    if (boardStatus.current_call) {
      console.log(`   Current call: ${JSON.stringify(boardStatus.current_call, null, 2)}`);
    }
  } else {
    console.log('⚠️ No live call board entry found');
  }
  
  console.log('\n' + '='.repeat(60));
  console.log('📋 TEST SUMMARY');
  console.log('='.repeat(60));
  console.log(`Lead ID: ${testLeadId}`);
  console.log(`Agent: ${TEST_AGENT_EMAIL}`);
  console.log(`Associate ID: ${associateId}`);
  console.log(`Phone: ${testPhone}`);
  console.log(`Market: Veteran (AOIntel)`);
  console.log(`VDP Call ID: ${insertedCall.id}`);
  console.log('\n🎯 EXPECTED BEHAVIOR:');
  console.log('   1. Lead should appear in AOIntel queue in Call Connector Pro');
  console.log('   2. Outbound dialer should pause (agent status = "in_call")');
  console.log('   3. Lead should have cnresolution = "pending"');
  console.log('   4. Lead should have aointel = true');
  console.log('   5. Lead should appear instantly via WebSocket (if server is running)');
  console.log('   6. Once resolved, lead should leave the AOIntel queue');
  console.log('\n💡 TROUBLESHOOTING:');
  console.log('   - If lead not created: Check server logs for AOIntel poller errors');
  console.log('   - If lead not appearing: Ensure WebSocket server is running');
  console.log('   - If dialer not pausing: Check agent_live_call_status table');
  console.log('='.repeat(60));
}

// Run the test
createTestPickup().catch(console.error);

