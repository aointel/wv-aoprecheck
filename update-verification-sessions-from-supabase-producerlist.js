...
async function updateVerificationSessionsFromSupabaseProducerlist() {
  try {
    console.log('🚀 Updating verification sessions with current MGA assignments from Supabase producerlist...');
    ...
    // 3. Look up current MGA teams from Supabase producerlist
    ...
    if (producerResult.error) {
      console.error('❌ Error fetching producerlist data:', producerResult.error);
      return;
    }

    console.log(`📋 Found ${producerResult.data.length} agents in Supabase producerlist table`);

    // 4. Create lookup map
    const mgaLookup = new Map();
    producerResult.data.forEach(agent => {
    ...
    console.log(`\n📊 Current MGA assignments from Supabase producerlist:`);
    ...
    console.error('❌ Error updating verification sessions:', error);
  }
}

// Run the update
updateVerificationSessionsFromSupabaseProducerlist();
