/**
 * Delete all call analytics and import everything from CSVs EXCEPT 1/27/2026
 */
...
async function deleteAllAndImport() {
  ...
  try {
    ...
    // Now import everything EXCEPT 1/27/2026
    console.log('📥 Importing all entries from CSVs EXCEPT 1/27/2026...\n');
    ...
    for (const csvFile of csvFiles) {
      ...
      for (const row of rows) {
        ...
        // Skip entries from 1/27/2026
        const dateStr = row.Date || '';
        if (dateStr.includes('1/27/2026') || dateStr.includes('01/27/2026')) {
          console.log(`   ⏭️ Skipping ${row.Name} (${row.Phone}) - date is 1/27/2026`);
          continue;
        }
      ...
      console.log(`   ✅ Found ${rows.length} transfers (after filtering 1/27)`);
    }

    console.log(`\n📊 Total transfers to import (excluding 1/27): ${allTransfers.length}\n`);
...
deleteAllAndImport().catch(console.error);
