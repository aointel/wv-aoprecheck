-- COMPLETE SUPABASE MIGRATION SCRIPT - FINAL VERSION
-- This migrates all ConnectNow tables from local PostgreSQL to Supabase
-- Fixed to avoid ALL foreign key type conflicts
...
-- =====================================================
-- 3. HOTLEADS SYSTEM (NO FOREIGN KEYS)
-- =====================================================
...
-- Hot lead notifications (removed foreign key)
CREATE TABLE IF NOT EXISTS hot_lead_notifications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  agent_email TEXT NOT NULL,
  hot_lead_id INTEGER, -- Reference without foreign key constraint
...
-- Hot lead callbacks (removed foreign key)
...
-- =====================================================
-- 4. SUPABASE MASTERLEAD TABLE
-- =====================================================

-- Masterlead table for Supabase (the main leads table)
...
CREATE TABLE IF NOT EXISTS call_center_events (
  ...
  call_id TEXT, -- Text reference instead of foreign key
...
CREATE TABLE IF NOT EXISTS appointment_reminders (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  appointment_id TEXT NOT NULL, -- Text reference instead of foreign key
...
-- =====================================================
-- 7. VERIFICATION AND VIDEO SYSTEMS
-- =====================================================
...
-- =====================================================
-- 8. GAMIFICATION AND USER EXPERIENCE
-- =====================================================
...
CREATE INDEX IF NOT EXISTS idx_masterlead_state ON masterlead(state);
...
-- Create permissive policies (restrict these later based on your security needs)
CREATE POLICY "Enable all access" ON users FOR ALL USING (true);
CREATE POLICY "Enable all access" ON agent_profiles FOR ALL USING (true);
...
CREATE POLICY "Enable all access" ON outbound_calls FOR ALL USING (true);
...
-- =====================================================
-- 13. TRIGGERS FOR AUTO-UPDATING TIMESTAMPS
-- =====================================================
...
CREATE TRIGGER update_masterlead_updated_at 
  BEFORE UPDATE ON masterlead 
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- Insert sample hotleads (check if table exists first)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'hotleads') THEN
...
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'masterlead') THEN
...
-- Success message
SELECT 'Supabase migration completed successfully! All tables created without foreign key conflicts.' as migration_status;