/**
 * Check CUSTOMERS table with correct columns
 */
...
async function checkCustomers() {
  ...
  try {
    // Search for Leyna by company_email
    const { data: leyna, error } = await supabase
      .from('customers')
      .select('*')
      .ilike('company_email', '%leynatran%');
    ...
    if (leyna.length === 0) {
      ...
      console.log('   THIS IS THE PROBLEM - SHE NEEDS TO BE ADDED!');
    } else {
    ...
    console.log('═'.repeat(70));
    console.log('🔍 COMPARING WITH CNSYSOP (WORKING ACCOUNT):\n');
    ...
    console.log(`📊 CNSYSOP IN CUSTOMERS: ${cnsysop?.length || 0} records\n`);
